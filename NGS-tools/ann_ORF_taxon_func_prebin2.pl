...
use Getopt::Std;
getopts("i:r:a:o:e:d:s:t:s:x:p:q:",\%opts);
...
my $cutoff_p     = $opts{q}; 
   $cutoff_p     = 0.5 unless defined($cutoff_p);
...
while($ll=<TMP>) {
  ...
  my ($tid, $rank, @lls) = split(/\t/, $ll);
  next unless ($rank eq "toprank");
  $tid_2_sptid{$tid} = $lls[16]; #### overwrite existing tid info
...
foreach $round (qw/1 2/) {
  ...
  foreach $sid (@all_sids) {
    ...
    foreach $orf_id (@orf_ids) {
      ...
      if ($no1 > 0) { 
        foreach $i (@t_taxids) { 
          $taxid_score{$i} += $score / $no1; 
          my $sptid = $tid_2_sptid{$i};
          $sptid_score{$sptid} += $score / $no1;
        } 
      }
    }
    my $cutoff_score = $sum_score * $cutoff_p;

    next unless (keys %taxid_score);
    my @taxid_score = keys %taxid_score; @taxid_score = sort {$taxid_score{$b} <=> $taxid_score{$a}} @taxid_score;
    my @sptid_score = keys %sptid_score; @sptid_score = sort {$sptid_score{$b} <=> $sptid_score{$a}} @sptid_score;
    
    foreach $i (@taxid_score) { 
      my $sptid = $tid_2_sptid{$i};
      print LOG "\ttaxid\t$i\t$taxid_score{$i}\tsptid:$sptid\n";
    }
    ...
    elsif ($#sptid_score ==0) { #### only one sptid
      $tid = $taxid_score[0];
    }
    elsif ( $sptid_score{ $sptid_score[0]} > $cutoff_score ) { #### first sptid score > 2nd
      foreach $i (@taxid_score) { 
        my $sptid = $tid_2_sptid{$i};
        if ($sptid eq $sptid_score[0]) {
          $tid = $i; last;
        }
      }
    }
